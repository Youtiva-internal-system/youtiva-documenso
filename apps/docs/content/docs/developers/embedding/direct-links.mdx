...
// URL: https://sign.youtiva.com/d/{token}
````
</Tab>
</Tabs>
...
```
https://sign.youtiva.com/d/{token}
```
...
```html
<iframe
  src="https://sign.youtiva.com/embed/direct/abc123xyz"
  width="100%"
  height="800"
  frameborder="0"
...
function DocumentSigner({ token }: { token: string }) {
  return (
    <div className="h-[800px] w-full">
      <iframe
        src={`https://sign.youtiva.com/embed/direct/${token}`}
...
function redirectToSigning(token: string) {
  window.location.href = `https://sign.youtiva.com/d/${token}`;
}
```
...
```
https://sign.youtiva.com/d/abc123xyz?externalId=order-12345
```
...
function getSigningUrl(token: string, orderId: string) {
  ...
  return `https://sign.youtiva.com/d/${token}?${params.toString()}`;
}
...
```
https://sign.youtiva.com/embed/direct/abc123xyz?externalId=order-12345
```
...
Enterprise plans support white-label embedding with the "Powered by Documenso" badge removed. [Contact sales](mailto:support@youtiva.com) for details.
...
function getEmbedUrl(token: string, orderId: string) {
  ...
  return `https://sign.youtiva.com/embed/direct/${token}?${params}`;
}
