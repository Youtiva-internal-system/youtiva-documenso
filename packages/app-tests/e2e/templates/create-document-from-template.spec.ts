...
test('[TEMPLATE]: should create a document from a template', async ({ page }) => {
  ...
  await page.getByLabel('Redirect URL').fill('https://sign.youtiva.com');
  ...
  await page.getByPlaceholder('Email').nth(1).fill('recipient2@youtiva.com');
  ...
  expect(document.documentMeta?.redirectUrl).toEqual('https://sign.youtiva.com');
...
test('[TEMPLATE]: should create a document from a template with custom document', async ({ page }) => {
  ...
  // Add a signer
  await page.getByPlaceholder('Email').fill('recipient@youtiva.com');
