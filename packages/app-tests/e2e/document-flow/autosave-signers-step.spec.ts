...
const addSignerAndSave = async (page: Page) => {
  await page.getByPlaceholder('Email').fill('recipient1@youtiva.com');
...
test.describe('AutoSave Signers Step', () => {
  test('should autosave the signers addition', async ({ page }) => {
    ...
    await expect(async () => {
      ...
      expect(retrievedRecipients[0].email).toBe('recipient1@youtiva.com');
  ...
  test('should autosave the signer update', async ({ page }) => {
    ...
    await page.getByPlaceholder('Email').fill('manager@youtiva.com');
    ...
    await expect(async () => {
      ...
      expect(retrievedRecipients.length).toBe(1);
      expect(retrievedRecipients[0].email).toBe('manager@youtiva.com');
  ...
  test('should autosave the signing order change', async ({ page }) => {
    ...
    await page.getByTestId('signer-email-input').nth(1).fill('recipient2@youtiva.com');
    await page.getByLabel('Name').nth(1).fill('Recipient 2');

    await page.getByRole('button', { name: 'Add Signer' }).click();

    await page.getByTestId('signer-email-input').nth(2).fill('recipient3@youtiva.com');
    ...
    await expect(async () => {
      ...
      const firstRecipient = retrievedRecipients.find((r) => r.email === 'recipient1@youtiva.com');
      const secondRecipient = retrievedRecipients.find((r) => r.email === 'recipient2@youtiva.com');
      const thirdRecipient = retrievedRecipients.find((r) => r.email === 'recipient3@youtiva.com');
